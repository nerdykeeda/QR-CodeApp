...
class LinkShortener {
    ...
    async shortenUrl() {
        ...
        console.log('Elements found:', { urlInput, resultSection }); // Debug log
        ...
        // Hide result section and show progress bar while processing
        if (resultSection) {
    ...
    async deleteLink(linkId) {
        ...
        try {
            const response = await fetch(`${this.baseUrl}/api/links/${linkId}?userId=anonymous`, {
                method: 'DELETE',
                headers: {
                    'X-API-Key': this.apiKey
                }
    ...
    showSuccess(data) {
        ...
        if (resultSection && shortenedUrl) {
            ...
            if (data.link && data.link.shortUrl) {
                // data.link.shortUrl is now just the short code, so construct full URL
                fullShortUrl = `${baseUrl}/${data.link.shortUrl}`;
                shortenedUrl.value = fullShortUrl;
            } else if (data.shortUrl) {
                // data.shortUrl is now just the short code, so construct full URL
